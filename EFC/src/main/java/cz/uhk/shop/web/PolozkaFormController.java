...
public class PolozkaFormController {
	...
	protected String onSubmit(@RequestParam(value="polId",required=false) Integer id, @ModelAttribute("polozka") Polozka command, BindingResult result) {
		//Provede se po odeslani formu
		polozkaValidator.validate(command, result);
		if (result.hasErrors()) {
			return "polozkaForm";
		}
		if (id!=null) {
